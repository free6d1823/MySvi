...
#ifdef CONFIG_ARMV7M_NO_PRINT_64BIT
...
static char *put_number(char *string, uint64_t number, uint8_t base, bool upper)
{
	const char *digits;
			   ...
			   int32_t precision, uint8_t type)
{
	...
	while (i <= --precision)
		ADDCH(string, '0');
	while (--i >= 0)
		ADDCH(string, reversed_string[i]);
	while (--width >= 0)
		ADDCH(string, ' ');
...
static char *output_buffer(char *pos, const char *end,
			   intptr_t type, int len, const char *buf)
{
	int buflen;
...
int vsnprintf(char *string, size_t size, const char *format, va_list args)
{
	...
	for (; *format; ++format) {
		...
		case 'p':
			...
			p = va_arg(args, void *);
			pos = format_number(pos, end, (uintptr_t)p,
...
int sprintf(char *buf, const char *fmt, ...)
{
    ...
    va_start(args, fmt);
    len = vsnprintf(buf, INT32_MAX, fmt, args);
    va_end(args);
