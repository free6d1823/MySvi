...
{
  return (ulong)tsc_hw_read_counter() - base;
}
...
} xyz __cmdload_section_data;

#define xyzModem_CHAR_TIMEOUT 6000 /* 2 seconds */
#define xyzModem_MAX_RETRIES 60
...
//        hdr_found = true;
//        break;
      if (c == SOH) {xyz.total_SOH++; hdr_found = true;}
      if (c == STX) {xyz.total_STX++; hdr_found = true;}
//      case CAN:
//        xyz.total_CAN++;
...
int xyzModem_stream_read(char *buf, int size, int *err)
{
  ...
  while (!xyz.at_eof && (size > 0))
  {
    ...
    if (!xyz.at_eof)
    {
      ...
      if (size < len)
        len = size;
      extern void *cmd_load_memcpy(void *dest, const void *src, size_t count);
      cmd_load_memcpy(buf, xyz.bufp, len);
      size -= len;
      buf += len;
      total += len;
      xyz.len -= len;
...
{
  ZM_DEBUG(diag_printf("xyzModem - %s mode, %d(SOH)/%d(STX)/%d(CAN) packets, %d retries\n",
