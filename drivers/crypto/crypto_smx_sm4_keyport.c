...
int sm4_function_test_keyport(unsigned char *psrc, unsigned char *pdst, int alg_flag, int sm4_mode,  int proc_len)
{
	int i;
	int ret;
	int outlen=16;
	...
	ret = str_cmp(pdst, dst, proc_len);
        if(ret) {
                printf("the key come from key port!\n");
		return 0;
	} else{
		printf("the key come from key context buffer!\n");
		return -1;
		}
	return 0;
}
...
int se_smx_sm4_keyport()
{
	...
	/*=====================================SM4-ECB============================================*/
   	/***********************calculate sm4-ecb********************************/
       	ret = sm4_function_test_keyport(psrc, pdst, SM4_FLAG, ECB_MODE, len);
        if(!ret){
	...
	if(!ret)
		printf("smx module key_iv set case pass \n");
	ret = sm4_function_test_keyport(psrc, pdst, SM4_FLAG, CBC_MODE, len);
        if(!ret){
