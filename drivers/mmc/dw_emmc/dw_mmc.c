...
static int dw_prepare(int lba, uintptr_t buf, size_t size)
{
	...
	flush_dcache_area((void *)buf, size);
	...
	flush_dcache_area((void *)dw_params.desc_base,
...
static int dw_read(int lba, uintptr_t buf, size_t size)
{
	...
	inval_dcache_area((void *)buf, size);
...
static int dw_write(int lba, uintptr_t buf, size_t size)
{
	/*buf data has already been written into lba by emmc dma mode*/
	return 0;
}
