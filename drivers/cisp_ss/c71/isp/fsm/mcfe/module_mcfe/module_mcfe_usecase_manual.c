...
#define FRAME_STREAM_RAW_FRAME_COUNT ( BUFSET_RAW_FRAME_COUNT + 3 )
#define FRAME_STREAM_OUT_FRAME_COUNT 1//( BUFSET_OUT_FRAME_COUNT + 1 )
...
static int release_streams_frames_bufsets( module_mcfe_usecase_config_t *config, const int force )
{
    ...
    if ( ( config->config_states.curr & MODULE_MCFE_USECASE_RAW_BUFSET_CONFIGURED ) && config->bufset_raw ) {
        ...
        // Force flag is set on deinit state and when MCFE process this state
        // raw frame stream is already destroyed by remote sensor so releasing frames is not necessary
        if ( !force ) {
...
static int module_mcfe_usecase_config_impl( module_mcfe_usecase_config_t *config, module_mcfe_sensor_cfg_t *sensor_config, module_mcfe_output_cfg_t *output_config )
{
    ...
    if ( !config || !sensor_config || !output_config ) {
        LOG( LOG_ERR, "Failed to configure manual use-case. Input parameter pointer is NULL." );
        return MCFE_ERR_INV_PARM;
    }

    // Check if usecase configuration initialised
    if ( !config->initialized ) {
        LOG( LOG_ERR, "Failed to configure manual use-case. Use-case configuration data is not initialised." );
        return MCFE_ERR_NOT_INIT;
    }

    LOG( LOG_INFO, "Configuring Manual use-case (context id: %u, channels: %d )", config->slot_id, sensor_config->num_channel );

    // Check if output buffer queues have been configured and deallocate if true
...
static int module_mcfe_usecase_start_impl( module_mcfe_usecase_config_t *config )
{
    ...
    // Check configuration pointer
    if ( !config ) {
        LOG( LOG_ERR, "Failed to start manual use-case. Use-case configuration data pointer is NULL." );
        return MCFE_ERR_INV_PARM;
    }
    ...
    if ( config->config_states.curr != config->config_states.reqd ) {
        if ( config->config_states.curr != config->config_states.last ) {
            LOG( LOG_ERR, "Failed to start manual use-case (context id: %u, current config: %u, required config: %u). Use-case is not fully configured.",
                 config->slot_id,
...
static int module_mcfe_usecase_stop_impl( module_mcfe_usecase_config_t *config )
{
    ...
    // Check configuration pointer
    if ( !config ) {
        LOG( LOG_ERR, "Failed to stop manual use-case. Use-case configuration data pointer is NULL." );
        return MCFE_ERR_INV_PARM;
    }
    ...
    if ( config->config_states.curr != config->config_states.reqd ) {
        if ( config->config_states.curr != config->config_states.last ) {
            LOG( LOG_ERR, "Failed to stop manual use-case (context id: %u, current config: %u, required config: %u). Use-case is not fully configured.",
                 config->slot_id,
                 config->config_states.curr,
                 config->config_states.reqd );
            config->config_states.last = config->config_states.curr;
    ...
    // Stop slot
    module_mcfe_slot_stop( config->slot_id );
    if ( rc != MCFE_ERR_NONE ) {
...
static int module_mcfe_usecase_release_resources_impl( module_mcfe_usecase_config_t *config )
{
    ...
    // Check configuration pointer
    if ( !config ) {
        LOG( LOG_ERR, "Failed to release resources of manual use-case. Use-case configuration data pointer is NULL." );
        return MCFE_ERR_INV_PARM;
    }
...
static int module_mcfe_usecase_process_event( module_mcfe_usecase_config_t *config, module_mcfe_event_t event )
{
	...
	LOG( LOG_INFO, "%s: event: %d", __FUNCTION__, event );
    // Check configuration pointer
    if ( !config ) {
        LOG( LOG_ERR, "Failed process event for manual use-case. Use-case configuration data pointer is NULL." );
        return MCFE_ERR_INV_PARM;
    }

    // Check if usecase already started
    if ( !config->started ) {
        LOG( LOG_ERR, "Failed to process manual use-case event (context id: %u). Use-case is not started.", config->slot_id );
        return MCFE_ERR_NOT_RUN;
    }

    // Check if usecase configured
    if ( config->config_states.curr != config->config_states.reqd ) {
        LOG( LOG_ERR, "Failed to process manual use-case event (context id: %u, current config: %u, required config: %u). Use-case is not fully configured.",
             config->slot_id,
             config->config_states.curr,
             config->config_states.reqd );
        return MCFE_ERR_NOT_INIT;
    ...
    if ( event == MODULE_MCFE_EVENT_RAW_BUFFER_READY ) {
        aframe_t *empty_frame = NULL, *filled_frame = config->remote_sensor_frame;

        // Check if filled frame is set
        if ( !filled_frame ) {
            LOG( LOG_WARNING, "Failed to get filled buffers from the remote sensor (context id: %u). Current raw buffers will be reused.", config->slot_id );

            if ( module_mcfe_bufset_set_buf_status( config->bufset_raw, MCFE_BUFSET_STATUS_FILLED ) != MCFE_ERR_NONE ) {
                LOG( LOG_ERR, "Failed to set buffers of the buffer set to filled state (context id: %u).", config->slot_id );
            }
        } else {

            if ( module_mcfe_bufset_swap( filled_frame, &empty_frame, config->bufset_raw, MCFE_BUFSET_SWAP_FILLED_IN_EMPTY_OUT ) != MCFE_ERR_NONE ) {
                LOG( LOG_ERR, "Failed to swap buffers (context id: %u).", config->slot_id );

                filled_frame->state = AFRAME_STATE_EMPTY;
                frame_stream_isp_put_frame( filled_frame );

                config->remote_sensor_frame = NULL;

                return MCFE_ERR_GENERAL;
            }

            empty_frame->state = AFRAME_STATE_FULL;
		...
		LOG( LOG_NOTICE, "MCFE_EVENT_OUT_BUFFER_READY");
 ...
 * Init, deinit
 **************************************************/
int module_mcfe_usecase_init_manual( module_mcfe_usecase_config_t *config, mcfe_slot_id_t slot_id, int hist_position_is_be )
{
    // Check configuration pointer
    if ( !config ) {
        LOG( LOG_ERR, "Failed to initialise manual use-case. Use-case configuration data pointer is NULL." );
        return MCFE_ERR_INV_PARM;
    }
...
int module_mcfe_usecase_deinit_manual( module_mcfe_usecase_config_t *config )
{
    // Check configuration pointer
    if ( !config ) {
        LOG( LOG_ERR, "Failed to deinitialize manual use-case. Use-case configuration data pointer is NULL." );
        return MCFE_ERR_INV_PARM;
    }
