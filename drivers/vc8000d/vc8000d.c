...
static void decoder_isr(short unsigned int irq, void *dev_id)
{
	...
	irqc_clear_irq((irq_t)core_array[id].irq );
}
...
int vc8000d_start(int id)
{
	...
	irq_register_vector((sirq_t)core_array[id].irq, decoder_isr, (void*)id);
	
	printf("VC8000D core %d: enable interrupt %d\n", id, core_array[id].irq);
	irqc_enable_irq((irq_t) core_array[id].irq);
	/* start frame */
	value = SW_DEC_E;
	writel(wClr, (uintptr_t)(base + HANTRODEC_IRQ_STAT_DEC*4));
	return 0;
}
