		   ...
		   int *cur_devnump)
{
	...
	case 2:
		if (strncmp(argv[1], "inf", 3) == 0) {
			...
			if (usb_print_info_num(*cur_devnump)) {
	...
	default: /* at least 4 args */
		if (strcmp(argv[1], "read") == 0) {
			...
			printf("\nread: device %d block # "LBAFU", count %lu ... ",
			       *cur_devnump, blk, cnt);
			n = usb_blk_read_devnum(if_type, *cur_devnump, blk, cnt,
					    (ulong *)addr);
			...
			printf("\nwrite: device %d block # "LBAFU", count %lu ... ",
			       *cur_devnump, blk, cnt);
			n = usb_blk_write_devnum(if_type, *cur_devnump, blk, cnt,
					     (ulong *)addr);
