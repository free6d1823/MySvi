...
struct dwc2_priv {
	...
	//temp remove struct reset_ctl_bulk	resets;
};
...
{
	int ret;
	// temp remove struct dwc2_priv *priv = dev_get_priv(dev);

	////temp remove ret = reset_get_bulk(dev, &priv->resets);
	ret = 0;
	if (ret) {
	...
	////temp remove ret = reset_deassert_bulk(&priv->resets);
	if (ret) {
	////temp remove	reset_release_bulk(&priv->resets);
