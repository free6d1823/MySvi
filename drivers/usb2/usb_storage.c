 ...
 * when div_by > mul_by
 */
static lbaint_t lbaint_muldiv(lbaint_t block_count, lbaint_t mul_by, lbaint_t div_by)
{
	lbaint_t bc_quot, bc_rem;
...
{
	lbaint_t lbaint; /* number of blocks if 512bytes block size */
	...
	if (dev_desc->lba > 0L && dev_desc->blksz > 0L) {
		...
		lbaint = (lba * (dev_desc->blksz/512));
		/* round to 1 digit */
		/* 2048 = (1024 * 1024) / 512 MB */
		mb = lbaint_muldiv(lbaint, 10, 2048);
...
{
	if(usb_num <= usb_max_devs) {
		dev_print(&usb_dev_desc[usb_num]);
		return 0;
	}
	return 1;
}

ulong usb_blk_read_devnum(enum if_type if_type, int devnum, lbaint_t start,
			lbaint_t blkcnt, void *buffer)
{
	struct blk_desc *desc;
	ulong n;
	desc = &usb_dev_desc[devnum];
	n = usb_stor_read(desc, start, blkcnt, buffer);
	return n;
}

ulong usb_blk_write_devnum(enum if_type if_type, int devnum, lbaint_t start,
			lbaint_t blkcnt, const void *buffer)
{
	struct blk_desc *desc;
	ulong n;
	desc = &usb_dev_desc[devnum];
	return usb_stor_write(desc, start, blkcnt, buffer);
}
...
static int usb_stor_probe_device(struct usb_device *udev)
{
	...
	for (lun = 0; lun <= max_lun && usb_max_devs < USB_MAX_STOR_DEV;
		...
		if (usb_stor_get_info(udev, &usb_stor[start],
			...
			debug("partype: %d\n", blkdev->part_type);
			usb_max_devs++;
			debug("%s: Found device %p,udev->devnum=%d\n", __func__, udev,udev->devnum);
		}
	}
#endif
