		...
		const struct usb_endpoint_descriptor *desc)
{
	...
	spin_lock_irqsave(&dep->dwc->lock, flags);
	ret = __dwc3_gadget_ep_enable(dep, desc, ep->comp_desc, false, false);
	spin_unlock_irqrestore(&dep->dwc->lock, flags);
		...
		struct usb_request *request)
{
	...
	list_for_each_entry_2(r, &dep->request_list, list) {
		if (r == req)
			break;
	}

	if (r != req) {
		list_for_each_entry_2(r, &dep->req_queued, list) {
			if (r == req)
				break;
		}
		if (r == req) {
		...
		const struct dwc3_event_devt *event)
{
	...
	case DWC3_DEVICE_EVENT_DISCONNECT:
		dev_vdbg(dwc->dev, "device disconnect event happened\n");
		dwc3_gadget_disconnect_interrupt(dwc);
		break;
	case DWC3_DEVICE_EVENT_RESET:
		dev_vdbg(dwc->dev, "device reset event happened\n");
		dwc3_gadget_reset_interrupt(dwc);
		break;
	case DWC3_DEVICE_EVENT_CONNECT_DONE:
		dev_vdbg(dwc->dev, "device connect done event happened\n");
