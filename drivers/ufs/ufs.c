...
static bool ufshc_is_active(uintptr_t base)
{
	return (mmio_read_32(base + HCE) & HCE_ENABLE) ? true : false;
}

static void ufshc_stop(uintptr_t base)
{
	unsigned int data;
	...
	} while (data & HCE_ENABLE);
}

static void ufshc_reset(uintptr_t base)
{
	unsigned int data;
	if (ufshc_is_active(base))
		ufshc_stop(base);
...
static void ufs_enum_verify(void)
{
	/* 0 means 1 slot */
