...
static struct pci_host_bridge dw_bridge;
...
static struct pci_host_bridge *host_bridge_init(void)
{
	struct pci_host_bridge *bridge = &dw_bridge;
...
int dw_pcie_host_init(struct pcie_port *pp)
{
	...
	if (!pp->va_cfg0_base) {
		...
		if (!pp->va_cfg0_base) {
			printf("Error with ioremap in function\n");
			ret = -ENOMEM;
			goto error;
		}
	}
	...
	if (!pp->va_cfg1_base) {
		pp->va_cfg1_base = (void *)pp->cfg1_base;
		if (!pp->va_cfg1_base) {
			printf("Error with ioremap\n");
			ret = -ENOMEM;
			goto error;
		}
	}
                           ...
                           u32 *val,  enum pci_size_t pci_size)
{
	...
	return dw_pcie_rd_conf(bus, devfn, where, size, val);
                           ...
                           unsigned int where, u32 val, enum pci_size_t pci_size)
{
	...
	return dw_pcie_wr_conf(bus, devfn, where, size, val);
}
