...
{
   uint32_t uInBlockSizeFld   = QUPE_INF(pQupAddr, QUP_IO_MODES, INPUT_BLOCK_SIZE);
...
/** @brief Enables/disables the input fifo.
    @param[in]  pQupAddr Qup Block address.
    @param[in]  bInFifoEnabled bool indicating whether to enable the
                input fifo.
    @return Nothing.
  */
void
...
/** @fn bool HAL_qupe_GetOutputFifoEnabled(void)
...
{
   uint32_t uIoMode;
   uint32_t uBitShftEn, uPackEn, uUnpackEn;
...
{
   HAL_qupe_IOModeType eOutMode,eInMode;
   uint32_t uWrCntVal, uWrCntCurrVal;
...
{
   uint32_t uI2cStatusVal;
   uint32_t uOperationalVal;
   uint32_t uQupStateVal;
   uint32_t val;
