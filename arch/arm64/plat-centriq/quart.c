...
{									\
	...
	__clk_generate_mnd((caddr_t)PCC_UART_CMD_RCGR(UART_DM_PCC_UART(n)),\
...
static inline void __uart_dm_config_pad(uint8_t gpio, uint8_t dir,
					uint8_t pull, uint8_t func)
{
        gpio_config_port(gpio,
			 GPIO_ENCODE_CONFIG(func, dir, pull,
					    GPIO_DRIVE_MA(8)));
}
...
static void uart_dm_write_byte(uint8_t byte)
{
...
static bool uart_dm_read_poll(void)
{
	__uart_dm_rx_service(UART_CON_ID);
	return !!(BYTES_IN_RING() > 0);
}

static uint8_t uart_dm_read_byte(void)
{
	while (1) {
		__uart_dm_rx_service(UART_CON_ID);
		if (BYTES_IN_RING())
...
static void uart_dm_irq_init(bool enable)
{
...
static void uart_dm_irq_ack(void)
{
...
static void uart_dm_ctrl_init(void)
{
	__uart_dm_ctrl_init(UART_CON_ID);
}

uart_driver dm_uart_drvtable =
{
	"dm",
	uart_dm_read_byte,
	uart_dm_write_byte,
