...
typedef enum HAL_qupe_MiniCoreType
{
    ...
    HAL_QUPE_MINI_CORE_SMBUS_MASTER        = 0x3, /**<SMBus master controller.*/
    HAL_QUPE_MINI_CORE_FLD_MAX             = 0x3, /**<Biggest field value.    */
...
typedef struct HAL_qupe_HwInfoType
{
    ...
    uint32_t              uNumInputSlots;
    uint32_t              uNumOutputSlots;
    uint32_t              uNumOutputBlockSlots;
    uint32_t              uNumOutputBlocks;
    uint32_t              uNumInputBlockSlots;
    uint32_t              uNumInputBlocks;
    uint32_t              uHWVersion;
} HAL_qupe_HwInfoType;
...
typedef struct
{
    ...
    bool bClockAlwaysOn; /**< set to TRUE to run the SPI clock continuously from start to end of the transfer */
    bool bChipSelectAlwaysOn; /**< set to TRUE*/
    HAL_QUPE_SPI_ChipSelectPolarityType eCSPolarity; /**< the chip-select polarity */
    uint32_t nChipSelectId; /**< the chip select to use for this transfer */
    uint32_t nDeassertionClockTicks; /**< the number of clock ticks to wait between transfer units */
...
typedef struct
{
    uint8_t nNumBits; /**< the number of bits per transfer unit */
    HAL_QUPE_SPI_LoopbackModeType eLoopbackMode; /**< configures internal loopback mode */
    bool bReceivedDataPacked; /**< set TRUE if data is to be packed in the SPI input FIFO */
    bool bTransmitDataPacked; /**< set to TRUE if data is packed in the SPI output FIFO */
    bool bOutputBitShift; /**< set to TRUE to enable the "Output Bit Shift" function */
    uint32_t nNumInputTransfers; /**< the number of transfer units that will be read into the input FIFO */
    uint32_t nNumOutputTransfers; /**< the number of transfer units that will be sent from the output FIFO */
...
typedef struct
{
    uint32_t cmd_element_base;
    uint32_t cmd_element_index;
    uint32_t cmd_size_in_bytes;
...
// Like above, but only do the QUP_CONFIG register
void HAL_qupe_Configure_ConfigOnly
(
   uint8_t *pQupAddr
);
// Like above, but only do the I2C_MASTER_CONFIG register
void
...
(
    uint8_t        *pQupAddr,
    const uint32_t uErrFlagsBitmap
);
...
(
    uint8_t        *pQupAddr,
    const uint32_t uEnabledErrFlagsBm
);
...
(
    ...
    const uint32_t             uFSI2cBusClockReqKhz
);
...
/** @brief Checks for bus clear bit to be unset by qup core
    @param[in]  pQupAddr qup block address.
    @return Nothing.
  */
bool
...
/** @brief Packs the operational state for debug.
  ...
  */
uint32_t HAL_qupe_Pack28_OperationalState
(
    uint8_t              *pQupAddr
);
 ...
 *                          parameters.
 */
void HAL_qupe_Spi_SetDeviceParameters(uint8_t*pQupAddr,
...
(
    uint8_t       *uPhysicalQupAddr,
    uint8_t       *uVirtualQupAddr,
...
I2cResult I2CPLATDEV_ResetTarget(I2CDEVQUP_Device *pDev);
