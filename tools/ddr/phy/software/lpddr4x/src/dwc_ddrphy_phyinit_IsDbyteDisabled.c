    ...
    if (NumCh==2 ) { 
        if (pUserInputBasic->NumActiveDbyteDfi1 == 0 ) { // only dfi0 (ChA) is enabled, dfi1 (ChB) disabled
            DisableDbyte = (DbyteNumber > pUserInputBasic->NumActiveDbyteDfi0-1 ) ? 1:0;
                ...
                DisableDbyte = ((pUserInputBasic->NumActiveDbyteDfi0-1 < DbyteNumber) && (DbyteNumber < (floor(NumDbyte/2)))) ? 1 :
                               (DbyteNumber > (floor(NumDbyte/2)+pUserInputBasic->NumActiveDbyteDfi1-1) ) ? 1:0;
        } 
    } 
    else if (NumCh == 1) {
        DisableDbyte = (DbyteNumber > pUserInputBasic->NumActiveDbyteDfi0-1 ) ? 1:0;
