...
#else
int dhrystone_main (caddr_t ptr)
#endif
/*****/
  ...
  /* Main and Proc_0 in the Ada version             */
{
  ...
  enum e_runmode runmode = scheduler;
  if (ptr == (caddr_t)-1) {
  	runmode = command;
  }
...
#undef printf
#endif
		printf("ts@%lld Dhrystones per Second=%6.1f\n", End_Time, Dhrystones_Per_Second);
	}
  }
...
int dhry(int argc, char *argv[])
{
	dhrystone_main((caddr_t)-1);
	return 0;
}
...
#ifndef HOSTED
__define_testfn(dhrystone_main, 16, SMP_CACHE_BYTES, 0, 0, 0);

MK_CMD(dhrystone, dhry, "run dhrystone bench",
	"dhrystone\n"
);
#endif
