...
#define do_fprintf		__fprintf
#define do_spin_lock(__lock)
#define do_spin_unlock(__lock)
#endif
...
#define get_linpack		get_percpu(linpack_ctx)
#endif
...
{
	startUsecs = linpack_time();
}

static inline void end_time(void)
{
	usecs = linpack_time() - startUsecs;
}
